...
public class BirthdayGrassScatter : EditorWindow
{
    ...
    [SerializeField] private Mode _mode = Mode.TerrainDetails;
    [SerializeField] private bool[] _layerMask = new bool[0];
    [SerializeField] private float _threshold = 0.45f;
    [SerializeField] private float _maxSlope = 32f;
    [SerializeField] private float _minHeight = 0f;
    [SerializeField] private float _patchScale = 0.035f;
    [SerializeField] private float _patchStrength = 0.55f;
    [SerializeField] private int _seed = 12345;

    // ---- detail mode
    [SerializeField] private GameObject _detailPrefab;
    [SerializeField] private Texture2D _detailTexture;
    [SerializeField] private bool _useBillboard;
    [SerializeField] private int _maxDensity = 5;
    [SerializeField] private float _minSize = 0.7f;
    [SerializeField] private float _maxSize = 1.4f;
    [SerializeField] private Color _healthy = new(0.55f, 0.72f, 0.36f);
    [SerializeField] private Color _dry = new(0.72f, 0.70f, 0.38f);
    [SerializeField] private bool _swapAxes;
    [SerializeField] private bool _flipX;
    [SerializeField] private bool _flipZ;
    [SerializeField] private float _detailDistance = 160f;
    [SerializeField] private bool _useInstancing;

    // ---- prefab mode
    [SerializeField] private GameObject _grassPrefab;
    [SerializeField] private float _spacing = 2.2f;
    [SerializeField] private float _jitter = 0.8f;
    [SerializeField] private int _maxInstances = 4000;
    [SerializeField] private Vector2 _scaleRange = new(0.8f, 1.35f);
    ...
    private void OnGUI()
    {
        ...
        else
        {
            ...
            // Spell out the current selection. A tick you did not notice getting
            // cleared is the single easiest way to waste an hour here.
            var on = layers.Where((l, i) => _layerMask[i])
                           .Select(l => l != null ? l.name : "?")
                           .ToArray();

            EditorGUILayout.HelpBox(
                on.Length == 0
                    ? "Nothing ticked, so no grass will be placed anywhere."
                    : "Grass will use: " + string.Join(", ", on),
                on.Length == 0 ? MessageType.Error : MessageType.Info);
        }
    ...
    private void DrawDetailOptions()
    {
        ...
        EditorGUILayout.HelpBox(
            "Unity's detail map indexing is not consistent across versions, so the grass can come out " +
            "mirrored or rotated even when the mask preview is perfect. Use Calibrate below to find the " +
            "right combination once, then forget about it.",
            MessageType.None);

        _swapAxes = EditorGUILayout.Toggle("Swap X/Z", _swapAxes);
        _flipX = EditorGUILayout.Toggle("Flip X", _flipX);
        _flipZ = EditorGUILayout.Toggle("Flip Z", _flipZ);

        if (GUILayout.Button("Calibrate Orientation (test patch)"))
            ScatterDetails(true);
    ...
    /// <summary>
    /// Maps a detail cell to a normalised terrain position, honouring the three
    /// orientation toggles. Every possible mirror and rotation is reachable from
    /// some combination of the three, so calibration always converges.
    /// </summary>
    private void CellToNormalised(int i, int j, int dw, int dh, out float nx, out float nz)
    {
        float fi = (i + 0.5f) / dw;
        float fj = (j + 0.5f) / dh;
    ...
    private void ScatterDetails(bool calibrate = false)
    {
        var td = _terrain.terrainData;
        ...
        for (int i = 0; i < dw; i++)
        {
            for (int j = 0; j < dh; j++)
            {
                CellToNormalised(i, j, dw, dh, out float nx, out float nz);

                // Calibration ignores every filter and fills exactly one quarter
                // of the map, so where it lands tells you the true orientation.
                if (calibrate)
                {
                    if (nx < 0.5f && nz > 0.5f) { map[i, j] = _maxDensity; placed += _maxDensity; }
                    continue;
                }
        ...
        {
            Debug.Log(
                "<b>[Birthday] Calibration patch placed.</b>\n\n" +
                "  Look straight down at the terrain in the Scene view (hold the right mouse button " +
                "and press Q/E, or click the Y cone on the axis gizmo in the top right).\n\n" +
                "  The patch should be in the <b>top left quarter</b> of the terrain: low X, high Z. " +
                "In that top-down view, +X is to the right and +Z is up.\n\n" +
                "  If it is somewhere else, change the toggles and press Calibrate again:\n" +
                "    top right     -> tick Flip X\n" +
                "    bottom left   -> tick Flip Z\n" +
                "    bottom right  -> tick both Flip X and Flip Z\n" +
                "    mirrored across the diagonal -> tick Swap X/Z\n\n" +
                "  Once the patch lands in the top left, press Scatter Grass and it will be correct.");
            return;
        }
        ...
        Debug.Log($"<b>[Birthday]</b> Scattered roughly <b>{placed:N0}</b> grass tufts across " +
                  ...
                  $"Orientation: swap {_swapAxes}, flipX {_flipX}, flipZ {_flipZ}.");
    }
